...
function getSteps(t: (key: TranslationKey) => string) {
  return [
    {
      title: t("tutorial.pickSecret"),
      desc: t("tutorial.pickSecretDesc"),
      visual: (
    ...
    {
      title: t("tutorial.guessClues"),
      desc: t("tutorial.guessCluesDesc"),
      visual: (
        <div className="space-y-3">
          <div className="flex items-center gap-3">
            <span className="w-4 h-4 rounded-full bg-bull shrink-0" />
            <span className="text-sm"><strong className="text-bull">{t("tutorial.filledDot")}</strong> {t("tutorial.filledDotDesc")}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="w-4 h-4 rounded-full border-2 border-cow shrink-0" />
            <span className="text-sm"><strong className="text-cow">{t("tutorial.hollowDot")}</strong> {t("tutorial.hollowDotDesc")}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="w-4 h-4 rounded-full bg-bg-hover shrink-0" />
            <span className="text-sm"><strong className="text-text-dim">{t("tutorial.greyDot")}</strong> {t("tutorial.greyDotDesc")}</span>
          </div>
        </div>
      ),
    },
    {
      title: t("tutorial.example"),
      desc: t("tutorial.exampleDesc"),
      visual: (
        <div className="space-y-3">
          <div className="flex items-center gap-1.5 justify-center">
            {[
              { d: "1", color: "bg-cow/20 text-cow border-cow/30", label: t("tutorial.moveIt") },
              { d: "2", color: "bg-bull/20 text-bull border-bull/30", label: t("tutorial.right") },
              { d: "3", color: "bg-bg-elevated text-text-dim border-border", label: t("tutorial.nope") },
              { d: "4", color: "bg-bg-elevated text-text-dim border-border", label: t("tutorial.nope") },
            ].map((c, i) => (
    ...
    {
      title: t("tutorial.useNotepad"),
      desc: t("tutorial.useNotepadDesc"),
      visual: (
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-[10px] text-text-dim">{t("tutorial.slotsFound")}</p>
        </div>
      ),
    },
  ];
